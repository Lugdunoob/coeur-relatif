#!/usr/bin/env bash
# Loop à contexte frais pour une mission de plan (cadrage/spec/architecture) : identique
# à ralph.sh, sauf qu'il ne marque jamais .loop/phase=lot. Le garde-fou garde-docs.sh
# bloque toute écriture dans docs/ quand phase=lot (pensé pour les lots de code) ; une
# mission de plan doit au contraire pouvoir écrire des cartes, la spec, l'architecture.
# Usage : bash .loop/ralph-plan.sh plan-lot7 15
set -euo pipefail
SLUG="${1:?slug, ex. plan-lot7}"; MAX_ITER="${2:-15}"
